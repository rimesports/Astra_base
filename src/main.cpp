...
extern "C" void vApplicationStackOverflowHook(TaskHandle_t xTask, char *pcTaskName)
{
    ...
    g_state.fault_flags |= FAULT_STACK_OVF;
    g_state.sys_state = SYS_FAULT;
    // Persist to RTC BKP so T:201 can report it after the upcoming IWDG reset
    ...
    RTC->BKP4R    = 0;
    RTC->BKP5R   += 1;
    RTC->BKP6R    = 1;   // fault type 1 = stack overflow
    __disable_irq();
    while (1) {}
}
...
{
    motor_ctrl_set_speed(0, 0);
    g_state.fault_flags |= FAULT_MALLOC_FAIL;
    g_state.sys_state = SYS_FAULT;
    RCC->APB1ENR |= RCC_APB1ENR_PWREN;
    PWR->CR      |= PWR_CR_DBP;
    RTC->BKP0R    = 0xDEADBEEFU;
    ...
    RTC->BKP4R    = 0;
    RTC->BKP5R   += 1;
    RTC->BKP6R    = 2;   // fault type 2 = malloc failed
    __disable_irq();
    while (1) {}
}
...
int main(void)
{
    ...
    xTaskCreate(control_task, "control", TASK_CONTROL_STACK, NULL, TASK_CONTROL_PRIO, &h_control);
    xTaskCreate(serial_task, "serial", TASK_SERIAL_STACK, NULL, TASK_SERIAL_PRIO, &h_serial);
    xTaskCreate(telemetry_task, "telemetry", TASK_TELEMETRY_STACK, NULL, TASK_TELEMETRY_PRIO, &h_telemetry);
...
static void control_task(void *arg)
{
    ...
    while (1)
    {
        ...
        // Apply pending PID gains from T:241 (reinitialises integrators)
        if (g_state.pid_gains_pending) {
            g_state.pid_gains_pending = false;
            pid_init(&pid_left,  g_state.pid_kp, g_state.pid_ki, g_state.pid_kd, g_state.pid_kf, -100.0f, 100.0f);
            pid_init(&pid_right, g_state.pid_kp, g_state.pid_ki, g_state.pid_kd, g_state.pid_kf, -100.0f, 100.0f);
        }
            ...
            (now - g_state.last_command_ms) > HEARTBEAT_TIMEOUT_MS)
        {
            ...
            g_state.fault_flags     |= FAULT_HEARTBEAT;
            g_state.heartbeat_fault_notify = true;
            pid_reset(&pid_left);
            pid_reset(&pid_right);
            profiled_l = 0.0f;
            profiled_r = 0.0f;
...
static void serial_task(void *arg)
{
    ...
    while (1)
    {
        // Send heartbeat-loss notification generated by control_task
        if (g_state.heartbeat_fault_notify) {
            g_state.heartbeat_fault_notify = false;
            char notif[80];
            snprintf(notif, sizeof(notif),
                     "{\"T\":999,\"fault\":\"heartbeat\",\"ms\":%lu}",
                     HAL_GetTick());
            serial_send_line(notif);
        }
