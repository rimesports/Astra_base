...
# Protects all stdout writes so the reader thread and main thread
# cannot interleave — prevents ANSI escape codes from clobbering
# partially-printed lines on the other thread.
...
def reader(ser, stop_event):
    ...
    while not stop_event.is_set():
        try:
            ...
            if line:
                with _stdout_lock:
                    # Clear current line (the "> " prompt), print received
                    # data, then reprint the prompt on a fresh line.
